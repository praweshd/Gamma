function [ripples] = FindGamma_I(signal,stdN,varargin)

%FindGamma_I - give in filtered LFP - detects gamma in time window. No wavelet
...
if ~isdmatrix(signal) | size(signal,2) ~= 2,
...
fourthPass = ripples;
mvmt_breaks = find(diff(signal(:, 1))>(1.9/frequency));
time_breaks = signal(mvmt_breaks, 1);
...
% disp(['After removing movements: ' num2str(size(ripples,1)) ' events.']);
...
% ripples(low_index, :) = [];
% 
% disp(['After power threshold: ' num2str(size(ripples,1)) ' events.']);
