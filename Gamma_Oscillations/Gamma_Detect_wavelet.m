function [gamma] = Gamma_Detect_wavelet(num_CH, spi_CH, varargin )
...
% Modified by Prawesh Dahal (June 10, 2019) 
...
% Spindle Detection
[gamma] = FindGamma_wav(restricted_lfp, 'thresholds', [spi_lowThresholdFactor spi_highThresholdFactor], 'durations', [interg maxg ming], 'frequency', 125); 

spindle_file = strcat(fbasename, '_', num2str(spi_CH), 'spindles_wav'); 
save(spindle_file, 'spindles');
spindle_events = strcat(fbasename,'_', num2str(spi_CH), '.zpi.evt');
channelID = spi_CH-1; 
